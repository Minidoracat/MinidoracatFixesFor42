...
- EN 描述每行加 description= 前綴（空行也輸出 description=），UTF-8 無 BOM
- 行尾用 CRLF：與遊戲內 Workshop 工具上傳時的回寫格式一致，避免每次上傳後 git
  都跳出只有行尾差異的假修改（.gitattributes 已對應設定，入庫仍正規化為 LF）
...
DST = os.path.join(REPO, "MOD", "MinidoracatFixesFor42", "workshop.txt")

TITLE = "Minidoracat Fixes for B42"
TAGS = "Build 42;Interface;Multiplayer"
VISIBILITY = "public"
