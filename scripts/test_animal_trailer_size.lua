...
--
-- 以最小 stub 模擬 PZ 端 API，驗證補欄位邏輯與「補資料出錯不得連累原版選單」的契約。
...
local function newBody(animalType, animalSize, modData)
    return {
        modData = modData or {},
        isAnimal = function(self) return animalType ~= nil end,
        getAnimalType = function(self) return animalType end,
        getAnimalSize = function(self) return animalSize end,
        getModData = function(self) return self.modData end,
    }
end

local function newSquare(bodies)
    return { getDeadBodys = function(self)
...
-- ── 載入待測檔 ─────────────────────────────────────────────────
assert(loadfile(MOD_LUA), "找不到 " .. MOD_LUA .. "（請從 repo 根目錄執行）")()
...
-- 6. 補資料炸掉時，原版選單仍須正常跑完（這正是本修復要解決的失效模式）
local boom = { getSquare = function() error("boom") end }
assert(ISVehicleMenu.doAnimalSubMenu({}, player, boom) == "vanilla-ran")
assert(vanillaCalls == 2)

-- 7. 手上拿著的屍體也要補（原版 ISVehicleMenu.lua:755 同樣沒防護）
...
print("MDFX_AnimalTrailerSize: 7 checks OK")
